package com.entity.view;

import com.entity.QixiebanEntity;
import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;
import java.io.Serializable;
import java.util.Date;

/**
 * 器械班
 * 后端返回视图实体辅助类
 * （通常后端关联的表或者自定义的字段需要返回使用）
 */
@TableName("qixieban")
public class QixiebanView extends QixiebanEntity implements Serializable {
    private static final long serialVersionUID = 1L;

		/**
		* 器械班类型的值
		*/
		private String qixiebanValue;



	public QixiebanView() {

	}

	public QixiebanView(QixiebanEntity qixiebanEntity) {
		try {
			BeanUtils.copyProperties(this, qixiebanEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



			/**
			* 获取： 器械班类型的值
			*/
			public String getQixiebanValue() {
				return qixiebanValue;
			}
			/**
			* 设置： 器械班类型的值
			*/
			public void setQixiebanValue(String qixiebanValue) {
				this.qixiebanValue = qixiebanValue;
			}
















}
